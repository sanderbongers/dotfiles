...
# Caches the generated abbrs until a formula is upgraded or removed.
set -l cache $__fish_cache_dir/gnubin.fish
if not test -f $cache; or test /opt/homebrew/opt -nt $cache; or test /usr/local/opt -nt $cache
    ...
    for name in (path basename $tools | string match -r '^[a-z0-9.-]+$') # Skip names like [
        builtin -q $name; and continue # Keep fish builtins (echo, pwd, realpath, test, ...)
    ...
    if set -q names[1]
        ...
        printf '%s\n' \
            ...
            "abbr -a man:gnubin --command man --command gman --regex '$pattern' --function __gnubin_expand_bare" >$cache
    else
        echo -n >$cache
    end
end
